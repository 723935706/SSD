...
__all__ = [
    'ShuffleNetV2_cbam', 'shufflenet_v2_x0_5', 'shufflenet_v2_x1_0',
...
class InvertedResidual(nn.Module):
    ...
    def forward(self, x):
        if self.stride == 1:
            ...
            temp = self.ca(self.branch2(x2)) * self.branch2(x2)
            temp = self.sa(temp) * temp
            out = torch.cat((x1, temp), dim=1)
            #out = torch.cat((x1, self.branch2(x2)), dim=1)

        else:
            temp = self.ca(self.branch2(x)) * self.branch2(x)
            temp = self.sa(temp) * temp
            out = torch.cat((self.branch1(x), temp), dim=1)
...
def _shufflenetv2(arch, pretrained, *args, **kwargs):
    model = ShuffleNetV2_cbam(*args, **kwargs)
...
@registry.BACKBONES.register('shufflenet_v2_cbam')
def shufflenet_v2_cbam(cfg, pretrained=False):
